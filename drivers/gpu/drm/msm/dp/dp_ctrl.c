...
static int dp_ctrl_update_phy_vx_px(struct dp_ctrl_private *ctrl,
			...
			int *training_step, enum drm_dp_phy dp_phy)
{
	...
	*training_step = DP_TRAINING_1;
	...
	ret = dp_ctrl_update_phy_vx_px(ctrl, dp_phy);
	if (ret)
		return ret;
					   ...
					   enum drm_dp_phy dp_phy)
{
	...
	dp_ctrl_train_pattern_set(ctrl, DP_TRAINING_PATTERN_DISABLE, dp_phy);
			...
			int *training_step, enum drm_dp_phy dp_phy)
{
	...
	*training_step = DP_TRAINING_2;
	...
	dp_ctrl_train_pattern_set(ctrl, pattern, dp_phy);
...
static int dp_ctrl_link_train_1_2(struct dp_ctrl_private *ctrl,
				   int *training_step, enum drm_dp_phy dp_phy)
{
	int ret;

	ret = dp_ctrl_link_train_1(ctrl, training_step, dp_phy);
	if (ret) {
	...
	ret = dp_ctrl_link_train_2(ctrl, training_step, dp_phy);
	if (ret) {
			      ...
			      int *training_step)
{
	...
	for (i = ctrl->link->lttpr_count - 1; i >= 0; i--) {
		...
		ret = dp_ctrl_link_train_1_2(ctrl, training_step, dp_phy);
		dp_ctrl_clear_training_pattern(ctrl, dp_phy);
	...
	ret = dp_ctrl_link_train_1_2(ctrl, training_step, DP_PHY_DPRX);
	if (ret) {
...
static int dp_ctrl_link_maintenance(struct dp_ctrl_private *ctrl)
{
	...
	dp_ctrl_clear_training_pattern(ctrl, DP_PHY_DPRX);
...
static bool dp_ctrl_send_phy_test_pattern(struct dp_ctrl_private *ctrl)
{
	...
	dp_ctrl_update_phy_vx_px(ctrl, DP_PHY_DPRX);
